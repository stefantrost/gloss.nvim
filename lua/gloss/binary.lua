...
function M.find()
	if vim.fn.executable("gloss") == 1 then
		return "gloss"
	end
	if vim.fn.executable(install_path) == 1 then
		return install_path
	end
	return nil
end
...
function M.ensure(callback)
	...
	local url =
		string.format("https://github.com/stefantrost/gloss/releases/latest/download/gloss_%s_%s", os_name, arch)
